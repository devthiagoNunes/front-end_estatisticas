...
export default ({tipo, categoria}) => {
  ...
  for(let i = 0; i < categoria.tipo.length; i++){
    dataGraficoMes.push({name: categoria.tipo[i], type: 'bar', data: [categoria.quantidade[i]]})
  ...
  const config3 = {
    grid: {
      ...
      left: categoria.quantidade.length > 12 ? '10%' : '8%',
    ...
    label: {
      ...
      fontWeight: categoria.quantidade.length > 12 ? 'bold' :   'normal',
      offset: categoria.quantidade.length > 12 ? [0, 18] : null,
      color: 'rgb(0, 0, 0)'
    },
    legend: { 
  ...
  return(
    <div className='grafico-mes' style={{
      ...
      {window.innerWidth >= 425 && window.innerWidth < 768 &&
        <Echarts option={config3} style={{height: categoria.quantidade.length > 12 ? '45vh' : '35vh', width: '140vw'}}
          opts={{renderer: 'canvas'}}
        />
      }

      {window.innerWidth >= 768 && window.innerWidth < 1024 &&
        <Echarts option={config2} style={{height: categoria.quantidade.length > 12 ? "120vh" : '40vh', width: "100vw"}} opts={{renderer:'canvas'}}/>}

      {window.innerWidth >= 1024 && window.innerWidth <= 1366 && <Echarts option={config1} style={{height: categoria.quantidade.length > 12 ? '50vh' : "38vh", width: "90vw"}} opts={{renderer: 'canvas'}}/>}

      {window.innerWidth > 1366 && <Echarts option={config4} style={{height: categoria.quantidade.length > 12 ? "70vh" : "50vh", width: "80vw"}} opts={{renderer: 'canvas'}}/>}
    </div>
  )
}