...
import { FileCsv } from 'phosphor-icons'
...
export default () => {
  ...
  const [natureCompany, setNatureConpany] = useState([])
  const [popupVisible, setPopoupVisible] = useState(false);

  useEffect(() => {
    const getPublicCompanyNature = async (filtros) => {
      const response = await getDataEmpresasAbertas(filtros);
      setNatureConpany(response.values)
    }

    const getActiveCompanyNature = async (filtros) => {
    ...
    const fetchNatureza = async () => {
      ...
      if(context.state.empresasAbertas)
        getPublicCompanyNature(filtros);
      else
        getActiveCompanyNature(filtros);
    }
  ...
  return(
    ...
    <div className="natureza">
      <div className='table'>
        ...
        <CreateTable table_name='Natureza' arr_dados={natureCompany} />
      </div>
    </div>
    </React.Fragment>
  )
