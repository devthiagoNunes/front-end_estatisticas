  ...
  useEffect(() => {
    const getEmpresasPorCapital = async () => {
      ...
      response.forEach(arr_data => {
        replyToCorporateNameParsed.push([arr_data[0], `R$ ${arr_data[1]}`])
      })
          ...
          <div className='table'>
            <p>
              <div style={{
                ...
                {`Classificação Por Razão Social`}
            ...
            <CreateTable table_name='Razao Social' arr_dados={replyToCorporateName} />
          </div>
        </div>
      </StyleCapitalSocial>
    </React.Fragment>
