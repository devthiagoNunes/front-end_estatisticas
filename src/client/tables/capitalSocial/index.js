...
import { getCapitalSocial } from '../../../services/pinot';
import { StyleCapitalSocial } from "./styled";

export default () => {

  const context = useContext(ContextGlobal)
  let [replyToCorporateName, setReplyToCorporateName] = useState([])
  const [empresasPorCapital, setEmpresasPorCapital] = useState([])
  ...
  useEffect(() => {
    const getEmpresasPorCapital = async () => {
      ...
      response.forEach(arr_data => {
        replyToCorporateNameParsed.push([arr_data[0], `R$ ${Math.floor(arr_data[1]/1000000000)} BI`])
      })

      setEmpresasPorCapital(response)
      setReplyToCorporateName(replyToCorporateNameParsed)
    }

    getEmpresasPorCapital();
  }, [context])
          ...
          <div className='table'>
            <p>
              <div style={{
                ...
                {`Classificação Por Capital Social`}
                <CSVLink className='icon-download-csv' data={dataToDownload} filename={"empresa-por-razao-social"} 
            ...
            <CreateTable table_name='Capital Social' arr_dados={replyToCorporateName} />
          </div>
        </div>
      </StyleCapitalSocial>
    </React.Fragment>
